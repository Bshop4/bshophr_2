		...
		<table width="100%">
			...
			<tr>
				<td colspan="2" style="text-align: left">薪酬单编号：${sid} <input
					type="hidden" name="salaryGrant.salaryGrantId" value="${sid}">
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td style="text-align: left">本机构总人数:${count } <input
					...
					，基本薪酬总数:${summap.salaryall }<input type="hidden"
					name="salaryGrant.salaryStandardSum" value="${summap.salaryall }" />
					，实发总额:<span id="salarySum_sum">${summap.salarypall }</span> <input
					type="hidden" id="salaryPaidSum" name="salaryGrant.salaryPaidSum"
					value="${summap.salarypall }" />


				</td>
				<td style="text-align: right;">登记人:<input type="text"
					name="salaryGrant.register" value="${userlogin.user_true_name }"
		...
		<table width="100%" border="1" cellpadding=0 cellspacing=1
			...
			<c:forEach items="${sgdlist}" var="s" varStatus="vs">
				...
				<input type="hidden" name="grantDetails[${vs.index }].salaryGrantId"
					value="${s.human_id }">
				<input type="hidden" id="salaryStandardSum${vs.count }"
					name="grantDetails[${vs.index }].salaryStandardSum"
					value="${s.salary_standard_sum }" />
				<tr class="TD_STYLE2">
					...
					<td><input type="text" name="bounsSum"
						id="bounsSum${vs.count }" onkeyup="onKeyPress('${vs.count }')"
						class="INPUT_STYLE2" /></td>
					<td><input type="text" name="saleSum" id="saleSum${vs.count }"
						onkeyup="onKeyPress('${vs.count }')" class="INPUT_STYLE2" /></td>
					<td><input type="text" name="deductSum"
						id="deductSum${vs.count }" onkeyup="onKeyPress('${vs.count }')"
						class="INPUT_STYLE2" /></td>
					<td><input type="text" name="salaryPaidSum"
						readonly="readonly" id="salaryPaidSum${vs.count }"
						value="${s.salary_standard_sum }" class="INPUT_STYLE2" /></td>
